...
function preprocess(logits, labels)
    out = logits[:, 2:(length(labels[1])+1), 1]
    for b in 2:size(logits, 3)
        out = hcat(out, logits[:, 2:(length(labels[b])+1), b])
    end
...
function training!(model, data, opt, SIDLabels, history)
    ...
    for (i, Xy) in enumerate(data)
        ...
        l, gs = Flux.withgradient(()->loss(cpu(model(input, Xy[3])), Xy[4], SIDLabels), Flux.params(model.classifier))

        Flux.update!(opt, Flux.params(model.classifier), gs)

        outputs = model(input, Xy[3]) |> cpu
...
function testing(model, data, SIDLabels, dir)
    ...
    for (i, Xy) in enumerate(data)
        ...
        CUDA.@allowscalar outputs = model(input, Xy[3]) 

        concat_arrays!(cpu(outputs), Xy[4], totalPredicts, totalTargets)
